...
{
	cout << "inside foilder dinesh" << endl;
	string path1 = "";
	if (en1 == "START")
	{
		path1 = dir_path;
		...
		//if(dir)
	{
				...
				//while((en=readdir(dir))!=NULL)
		{
			...
			{
				en2 = fs::absolute(entry.path()).string();
				cout << ":" << en2 << ":";
				...
				for (int i = 0; i < (sizeof(file_extension) / sizeof(file_extension[1])); i++)
				{
					...
					if (en2.find(file_extension[i]) == (en2.length() - file_extension[i].size()) && en2.find(file_extension[i]) < en2.length())
					{
						...
						/*if (fs::is_directory(en2))
						//if(d_name)
						{
							en2 = en2.substr(en2.find_last_of('\\') + 1);
							checkDir(path1, en2, key_word_list);
							present++;
						}
						*/
						//else
						//{
						cout << "Before en2 :" << en2 << endl;
						en2 = en2.substr(en2.find_last_of('\\') + 1);
						cout << "After en2:" << en2 << endl;
						checkValuesOfThefile(path1, en2, key_word_list[i]);
						cout << "hal dinesh143" << endl;
						present++;
				...
				{
					//en2 = en2.substr(en2.find_last_of('\\') + 1);
					checkDir(fs::absolute(entry.path()).string(),"START", key_word_list);
				}
